...
def get_cfbd_team_information(
    ...
    `api_key` (str, optional):
        ...
        If `api_key` is not null, this function will automatically
        assume that the inputted `api_key` is a valid CFBD API key.
...
def get_cfbd_team_matchup_history(
    ...
    if min_season is not None and min_season > (now.year + 1):
        raise ValueError(f"`min_season` cannot be greater than {min_season}.")
    elif min_season is not None and min_season < 1869:
        raise ValueError("`min_season` cannot be less than 1869.")

    if max_season is not None and max_season > (now.year + 1):
        raise ValueError(f"`max_season` cannot be greater than {max_season}.")
    elif max_season is not None and max_season < 1869:
        raise ValueError("`max_season` cannot be less than 1869.")
