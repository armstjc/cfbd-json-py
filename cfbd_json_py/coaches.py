...
def get_cfbd_coaches_info(
    ...
    if min_season != None and max_season == None:
        if season != None and min_season != season:
            ...
            logging.warning(
                "If you only want coaching information for a single season, and not for a range of seasons, only set `year` to the seaon you want coaching info for, and leave `min_season` and `max_season` as `None` (NULL)."
            )
            min_season = None
        elif season == None:
            logging.warning(
    ...
    if min_season != None and max_season != None:
        if season != None:
            ...
            raise ValueError("`min_season` cannot be greater than `max_season`.")
