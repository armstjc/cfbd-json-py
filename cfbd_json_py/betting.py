...
# Last Updated Date: 12/22/2023 03:17 PM EDT
...
def get_cfbd_betting_lines(
    ...
    ## `home` to `home_team`
    if home != None and home_team != None and (home != home_team):
        raise ValueError(
    ...
    ## `away` to `away_team`
    if away != None and away_team != None and (away != away_team):
        raise ValueError(
    ...
    # if conference != None and conference != None \
    ...
    if game_id != None and season != None:
        warnings.warn(
            ...
            + "only set `game_id` to the game ID, and leave `season` as `NULL`."
        )
    ...
    if (game_id == None) and (season == None) and (week != None):
        raise ValueError("When setting a value for `week`, `season` cannot be null.")

    # if cache_data == True and ((team != None) or (home_team != None) or (away_team != None) or (conference != None)):
    #     logging.warning(
