...
# Last Updated Date: 10/06/2023 07:55 PM EDT
...
def check_cfbd_api_compatability():
    ...
    cfbd_swagger_json_url = "https://raw.githubusercontent.com/CFBD/cfb-api/main/swagger.json"
    ...
    print(
        f"Package Compatability Version:\t{cfbd_json_py_version}\nCurrent CFBD API Version:\t{cfbd_api_version}")

    if cfbd_api_version == cfbd_json_py_version:
        print("Version match confirmed. No need to raise an exception.")
    else:
        print("Version match unconfirmed. Raising a `ValueError` exception to make it clear that there is a version mismatch.")
        raise ValueError(
            "There is a difference between the current version of the CFBD API, " +
            "and the version of the CFBD API this package is compatable with." +
